...
//       // Try to play the video if it exists
//       if (videoRef.current && currentItem?.type === "video") {
//         videoRef.current.play().catch(console.error);
//       }
//     }
//   }, [hasUserInteracted, currentItem?.type]);

//   // Add click/touch listeners to handle user interaction
//   useEffect(() => {
...
//     setTouchEndX(null);
//     setIsUserInteracting(true);
//     handleUserInteraction(); // Enable video playback on touch
//   }, [handleUserInteraction]);
...
//           key={index}
//           className="absolute inset-0 w-full h-full object-cover"
//           autoPlay={!deviceInfo.isIOS} // Disable autoplay on iOS initially
//           loop
//           muted
//           controls={false}
//           playsInline={true}
...
//       );
//     }
//   }, [getImageUrl, deviceInfo.isSlowNetwork, deviceInfo.isIOS, handleVideoCanPlay, handleVideoLoadedData]);
...
//       onTouchMove={handleTouchMove}
//       onTouchEnd={handleTouchEnd}
//       onClick={handleUserInteraction} // Handle click for desktop
//       style={{
//         // Prevent any iOS Safari video controls from showing
...
//       {/* Show a subtle overlay with text when video hasn't started on iOS */}
//       {deviceInfo.isIOS && currentItem?.type === "video" && !hasUserInteracted && (
//         <div className="absolute inset-0 flex items-center justify-center bg-black/30 pointer-events-none">
//           <div className="text-white text-center">
//             <div className="text-sm opacity-75">Tap to play video</div>
//           </div>
//         </div>
//       )}
...
const MediaSlideshowComponent: React.FC<MediaSlideshowProps> = ({
  ...
  // Add more aggressive event listeners for iOS
  useEffect(() => {
    if (typeof window === 'undefined') return;
    ...
    // Also try to play video on component mount after a short delay (for some iOS versions)
    const timer = setTimeout(() => {
      if (videoRef.current && currentItem?.type === "video" && deviceInfo.isIOS) {
        videoRef.current.play().catch(() => {
          // Silently fail, user interaction will be required
        });
      }
    }, 500);
  ...
  }, [handleUserInteraction, currentItem?.type, deviceInfo.isIOS]);
